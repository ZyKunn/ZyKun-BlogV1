<template><div><h1 id="_05-servlet-四" tabindex="-1"><a class="header-anchor" href="#_05-servlet-四" aria-hidden="true">#</a> 05 - Servlet（四） ✈️</h1>
<nav class="table-of-contents"><ul><li><router-link to="#请求转发与重定向">请求转发与重定向</router-link><ul><li><router-link to="#请求转发-forward">请求转发（forward）</router-link></li><li><router-link to="#重定向-redirect">重定向（redirect）</router-link></li></ul></li><li><router-link to="#servlet-作用域对象">Servlet 作用域对象</router-link><ul><li><router-link to="#request">request</router-link></li><li><router-link to="#session">session</router-link></li><li><router-link to="#servletcontext">ServletContext</router-link></li></ul></li><li><router-link to="#会话跟踪技术之-cookie">会话跟踪技术之 Cookie</router-link><ul><li><router-link to="#cookie-的使用如下">cookie 的使用如下：</router-link></li><li><router-link to="#cookie-的应用场景">Cookie 的应用场景：</router-link></li><li><router-link to="#案例讲解-记住密码实现">案例讲解：记住密码实现</router-link></li></ul></li><li><router-link to="#作业">作业</router-link></li></ul></nav>
<h2 id="请求转发与重定向" tabindex="-1"><a class="header-anchor" href="#请求转发与重定向" aria-hidden="true">#</a> 请求转发与重定向</h2>
<p>Servlet 中的跳转技术主要包含两种：</p>
<ol>
<li>
<p>请求转发（request.getRequestDispartcher().forward(request,response)）</p>
</li>
<li>
<p>重定向（response.sendRedirect()）</p>
<p><img src="@source/notes/java/06/05-Servlet(四)/assets/1611626736388.png" alt="1611626736388"></p>
</li>
</ol>
<h3 id="请求转发-forward" tabindex="-1"><a class="header-anchor" href="#请求转发-forward" aria-hidden="true">#</a> 请求转发（forward）</h3>
<p>请求转发也称之为服务端跳转，即客户端只发送一次请求，后续的多次请求由服务端发起，因此客户端的请求地址栏只会显示第一次发送的地址（实际服务端已经在内部转发了多次），请求转发可以将存储在一次 request 范围(客户端发起的)之内的数据传递到下一个 Servlet 中</p>
<div class="language-JAVA line-numbers-mode" data-ext="JAVA"><pre v-pre class="language-JAVA"><code>//向request范围存储了一个属性（key:username,value:softeem）
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="重定向-redirect" tabindex="-1"><a class="header-anchor" href="#重定向-redirect" aria-hidden="true">#</a> 重定向（redirect）</h3>
<p>重定向也称之为客户端跳转，即客户端请求会发送多次，而且地址栏会显示最后一次请求的地址，由于重定向是客户端的跳转方式，因此每次请求之间没有直接关系，所以存储在 request 范围之内数据无法传递到下一个请求</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//request范围内数据无法传递到下一个servlet中</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="servlet-作用域对象" tabindex="-1"><a class="header-anchor" href="#servlet-作用域对象" aria-hidden="true">#</a> Servlet 作用域对象</h2>
<p><img src="@source/notes/java/06/05-Servlet(四)/assets/1611631738599.png" alt="1611631738599"></p>
<h3 id="request" tabindex="-1"><a class="header-anchor" href="#request" aria-hidden="true">#</a> request</h3>
<p>request 作用域作用于一次请求范围（客户端请求）,当使用请求转发跳转时，可以向 request 范围之内存储数据，并且在下一个 Servlet 中可以将 request 的数据获取</p>
<p>使用方式：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//存储数据到request</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="session" tabindex="-1"><a class="header-anchor" href="#session" aria-hidden="true">#</a> session</h3>
<p>session 也称之为会话，通常指的是一次项目的访问过程（只要浏览器不关闭，或者不到达 session 的有效期）存储在 session 范围之内的数据会一直存在。</p>
<p>session 有效期默认是:30 分钟（在 tomcat 的 web.xml 文件中有配置）:</p>
<p><img src="@source/notes/java/06/05-Servlet(四)/assets/1611629377155.png" alt="1611629377155"></p>
<blockquote>
<p>如果需要修改 session 的有效期,不要去直接修改服务器(Tomcat-&gt;conf 目录下)中的 web.xml；而是应该修改 web 项目中 WEB-INF 下的 web.xml</p>
</blockquote>
<p>客户端每一次服务端建立连接时都会创建一个 session（具备一个独一无二的 sessionid），在同一次访问中，无论页面（请求转发）如何跳转，只要 sessionid 是同一个，则 session 中的数据是一直有效且共享的。一旦浏览器关闭，下一次重新开启时，sessionid 会发生变更。</p>
<p>使用方式</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>作用：</strong></p>
<p>session 的存储范围为服务端，一般用于存储一些安全性要求较高的少量数据，比如：用户的登录信息（账号密码等）</p>
<h3 id="servletcontext" tabindex="-1"><a class="header-anchor" href="#servletcontext" aria-hidden="true">#</a> ServletContext</h3>
<p>ServletContext 也称之为上下文环境；一旦 web 容器启动，项目被装载后，服务端会自动创建一个上下文环境（SerlvetContext）,该环境是唯一的（单例）；只要服务器不重启，存储在在该范围内的数据会一直有效，并且可以实现多个用户之间的数据共享。</p>
<p>ServletContext 的获取包含三种方式:</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//方式一：</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用方式：</p>
...
<p>如果需要对指定作用域中的属性清除可以使用：</p>
<ul>
<li>removeAttribute(name)移除</li>
</ul>
<p>对于 session 来说还可以直接使用<code v-pre>invalidate()</code>方法，清除整个 session 对象</p>
</blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="会话跟踪技术之-cookie" tabindex="-1"><a class="header-anchor" href="#会话跟踪技术之-cookie" aria-hidden="true">#</a> 会话跟踪技术之 Cookie</h2>
<p>Http 协议是无状态的（每次一次的 http 请求相互之间无关联），但是实际开发中经常需要在多个请求之间进行数据的传递，以及状态的转移；此时 servlet 中支持四种会话跟踪技术：</p>
<ol>
<li>
<p><strong>URL 传值</strong>（get 请求）</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>http://localhost/javaweb01/login?username=admin&amp;password=123456&amp;code=0.12312435345345
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p><strong>隐藏域传值</strong>（post/get 请求）</p>
<div class="language-html line-numbers-mode" data-ext="html"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>input</span> <span class="token attr-name">type</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>hidden<span class="token punctuation">"</span></span> <span class="token attr-name">name</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>method<span class="token punctuation">"</span></span> <span class="token attr-name">value</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>login<span class="token punctuation">"</span></span> <span class="token punctuation">/></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
...
<p>Cookie 是客户端浏览器，实现数据缓存的一种技术，是通过在本地操作系统中以文件存储的形式，存储少量(4kb 左右)的数据信息；cookie 由服务端制造，通过 response 对象响应到客户端，然后再通过 request 发送到服务端，</p>
</li>
</ol>
<h3 id="cookie-的使用如下" tabindex="-1"><a class="header-anchor" href="#cookie-的使用如下" aria-hidden="true">#</a> cookie 的使用如下：</h3>
<ol>
<li>
<p>制造并存储 cookie</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@WebServlet</span><span class="token punctuation">(</span><span class="token string">"/cookie/add"</span><span class="token punctuation">)</span>
...
<li>
<p>读取 Cookie</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@WebServlet</span><span class="token punctuation">(</span><span class="token string">"/cookie/read"</span><span class="token punctuation">)</span>
...
<h3 id="cookie-的应用场景" tabindex="-1"><a class="header-anchor" href="#cookie-的应用场景" aria-hidden="true">#</a> Cookie 的应用场景：</h3>
<ol>
<li>大型网站的首页信息的缓存</li>
<li>购物车功能</li>
<li>保存账号功能</li>
...
<h3 id="案例讲解-记住密码实现" tabindex="-1"><a class="header-anchor" href="#案例讲解-记住密码实现" aria-hidden="true">#</a> 案例讲解：记住密码实现</h3>
<h2 id="作业" tabindex="-1"><a class="header-anchor" href="#作业" aria-hidden="true">#</a> 作业</h2>
<ol>
<li>基于 Cookie 以及 session，实现一个浏览记录保存的功能</li>
</ol>
<p><img src="@source/notes/java/06/05-Servlet(四)/assets/1611646163155.png" alt="1611646163155"></p>
</div></template>
