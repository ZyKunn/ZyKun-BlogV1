<template><div><h1 id="_02-io-二" tabindex="-1"><a class="header-anchor" href="#_02-io-二" aria-hidden="true">#</a> 02 - IO（二） 📝</h1>
<nav class="table-of-contents"><ul><li><router-link to="#io-综合练习">IO 综合练习</router-link></li><li><router-link to="#缓冲流-续">缓冲流(续)</router-link><ul><li><router-link to="#flush">flush()</router-link></li></ul></li><li><router-link to="#打印流-输出">打印流（输出）</router-link><ul><li><router-link to="#printstream">PrintStream</router-link></li><li><router-link to="#printwriter">PrintWriter</router-link></li></ul></li><li><router-link to="#randomaccessfile">RandomAccessFile</router-link><ul><li><router-link to="#构造器">构造器</router-link></li><li><router-link to="#常见方法-2">常见方法</router-link></li></ul></li><li><router-link to="#对象序列化">对象序列化</router-link><ul><li><router-link to="#序列化">序列化</router-link></li><li><router-link to="#反序列化">反序列化</router-link></li><li><router-link to="#transient-关键字">transient 关键字</router-link></li></ul></li></ul></nav>
<h2 id="io-综合练习" tabindex="-1"><a class="header-anchor" href="#io-综合练习" aria-hidden="true">#</a> IO 综合练习</h2>
<h2 id="缓冲流-续" tabindex="-1"><a class="header-anchor" href="#缓冲流-续" aria-hidden="true">#</a> 缓冲流(续)</h2>
<h3 id="flush" tabindex="-1"><a class="header-anchor" href="#flush" aria-hidden="true">#</a> flush()</h3>
<p>对于字符输出流，系统会自带缓冲区，默认 write 时，数据会先存储到缓冲区中，一旦执行 close()方法，此时缓冲区中的数据会全部同步到目标输出源中，若此时还不能 close，但是需要将数据实时同步时，可以通过调用输出流的 flush 方法，强制刷新:</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//        BufferedWriter bw = new BufferedWriter(new FileWriter("day2-IO02/c.txt"));</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="打印流-输出" tabindex="-1"><a class="header-anchor" href="#打印流-输出" aria-hidden="true">#</a> 打印流（输出）</h2>
<p>在 IO 中提供了两个用于输出的流：打印流；这两个流主要区别在于一个用于字符输出，另一个用于字节输出：</p>
<ul>
<li>PrintWriter</li>
<li>PrintStream</li>
</ul>
<h3 id="printstream" tabindex="-1"><a class="header-anchor" href="#printstream" aria-hidden="true">#</a> PrintStream</h3>
<p>字节打印流</p>
<h4 id="常见的构造器" tabindex="-1"><a class="header-anchor" href="#常见的构造器" aria-hidden="true">#</a> 常见的构造器</h4>
<ul>
<li>PrintStream(OutputStream os)</li>
<li>PrintStream(OutputStream os,boolean autoFlush)</li>
...
<h3 id="printwriter" tabindex="-1"><a class="header-anchor" href="#printwriter" aria-hidden="true">#</a> PrintWriter</h3>
<p>字符打印流</p>
<h4 id="常见构造器" tabindex="-1"><a class="header-anchor" href="#常见构造器" aria-hidden="true">#</a> 常见构造器</h4>
<ul>
<li>PrintWriter(Writer w)</li>
<li>PrintWriter(Writer out,boolean autoFlush)</li>
<li>PrintWriter(String fileName)</li>
</ul>
<h4 id="常见方法-1" tabindex="-1"><a class="header-anchor" href="#常见方法-1" aria-hidden="true">#</a> 常见方法</h4>
<ul>
<li>print(..)</li>
<li>println(..)</li>
<li>write(..)</li>
</ul>
<h2 id="randomaccessfile" tabindex="-1"><a class="header-anchor" href="#randomaccessfile" aria-hidden="true">#</a> RandomAccessFile</h2>
<p>RandomAccessFile 是 javaIO 中提供的一个功能丰富的特殊流，该流包含了对于文件的读写操作，具体的使用方式和 FileInputStream 与 FileOutputStream 几乎一致，除此之外，RandomAccessFile 还提供了对于元数据(原始数据类型)的读写操作。</p>
...
<li>RandomAccessFile(String fname,String mode)</li>
</ul>
<blockquote>
<p>其中第二个参数 mode 的可取值为：</p>
<ul>
<li>
<p>&quot;r&quot; 以只读方式打开。调用结果对象的任何 write 方法都将导致抛出 IOException。</p>
</li>
<li>
<p>&quot;rw&quot; 打开以便读取和写入。如果该文件尚不存在，则尝试创建该文件。</p>
</li>
<li>
<p>&quot;rws&quot; 打开以便读取和写入，对于 &quot;rw&quot;，还要求对文件的内容或元数据的每个更新都同步写入到底层存储设备。</p>
</li>
<li>
<p>&quot;rwd&quot; 打开以便读取和写入，对于 &quot;rw&quot;，还要求对文件内容的每个更新都同步写入到底层存储设备。</p>
</li>
</ul>
</blockquote>
<h3 id="常见方法-2" tabindex="-1"><a class="header-anchor" href="#常见方法-2" aria-hidden="true">#</a> 常见方法</h3>
<ul>
<li>read(byte[] b)</li>
<li>readInt()</li>
<li>readBoolean()</li>
<li>...</li>
<li>write(byte[] b,int offset,int len)</li>
<li>write(byte[] b)</li>
<li>writeInt(int i)</li>
<li>writeBoolean(boolean b)</li>
<li>getFilePointer() :获取当前文件的指针偏移位置</li>
<li>seek(int position)：设置文件指针到目标位置，在该位置发生下一次读写</li>
</ul>
<p>基本使用：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//创建一个随机访问文件流对象</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="综合案例-文件断点拷贝" tabindex="-1"><a class="header-anchor" href="#综合案例-文件断点拷贝" aria-hidden="true">#</a> 综合案例:文件断点拷贝</h4>
<p>实现原理:</p>
<p>文件正常拷贝的过程中，由于客观原因出现异常，导致中途停止拷贝；下一次拷贝时读取目标文件中上次拷贝的终止位置，将文件指针设置到该位置，继续拷贝。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="对象序列化" tabindex="-1"><a class="header-anchor" href="#对象序列化" aria-hidden="true">#</a> 对象序列化</h2>
<p>​ 目前为止所了解所有流，操作的一般都是文本文件(字符流)和二进制的文件(字节流)；但是如果需要将一个 java 对象写入文件中此时字节流与字符流都无法满足需求；在实际项目中经常会涉及到一些缓存技术，而缓存中(内存级别存储)可以存储 java 对象，当外部需要对象时直接从缓存中获取，减少对于数据的 IO 操作。</p>
<p>​ 因此，针对以上需求，java 中提供了对象序列化技术，允许将一个 java 对象存储到文件，或者通过网络发送到指定的目标主机；同时也可以通过文件(或者网络)直接读取这些存储的 java 对象，对于 java 对象的读写操作，IO 中称之为对象的序列化（写）与反序列化(读)。通过对象序列化机制可以最大限度的保证数据的完整性。</p>
<p>java 中用于进行对象序列化操作的两个流称之为对象流:</p>
<ul>
<li>ObjectOutputStream</li>
<li>ObjectInputStream</li>
</ul>
<p>对象实现序列化的前提条件为必须让目标类型实现以下两个接口中的一个：</p>
<ul>
<li><strong>Serializable（推荐）</strong></li>
...
<p>注意事项：</p>
<p>对于需要进行序列化操作的对象必须让其类实现<code v-pre>Serializable</code>接口：</p>
...
<h3 id="transient-关键字" tabindex="-1"><a class="header-anchor" href="#transient-关键字" aria-hidden="true">#</a> transient 关键字</h3>
<p>对象序列化本质就是以<strong>元数据</strong>的形式将对象中的属性值写入到文件中，对于方法并不会序列化；对于不需要序列化的属性可以使用<code v-pre>transient</code>关键字进行修饰。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code> <span class="token keyword">private</span> <span class="token keyword">transient</span> <span class="token keyword">int</span> power<span class="token punctuation">;</span>
