<template><div><h1 id="👻-参数校验注解不生效解决方案" tabindex="-1"><a class="header-anchor" href="#👻-参数校验注解不生效解决方案" aria-hidden="true">#</a> 👻 参数校验注解不生效解决方案</h1>
<hr/>
<nav class="table-of-contents"><ul><li><router-link to="#例-1-注解写在-controller-层的接口参数中">例 1：注解写在 Controller 层的接口参数中</router-link></li><li><router-link to="#例-2-注解写在实体中">例 2：注解写在实体中</router-link></li><li><router-link to="#补充-除-notnull-之外的其他校验相关的注解">补充：除@NotNull 之外的其他校验相关的注解</router-link></li></ul></nav>
<blockquote>
<p>在日常开发过程中，我们会写一些注解⽤于加在 java 的<strong>实体类</strong>属性字段上，或者放到 controller 层的方法参数中，在进⾏请求或请求的时候会根据字段的注解进⾏对应的校验，简化校验相关的代码。
当校验不通过时，会返回对应的 message 描述</p>
</blockquote>
<h3 id="例-1-注解写在-controller-层的接口参数中" tabindex="-1"><a class="header-anchor" href="#例-1-注解写在-controller-层的接口参数中" aria-hidden="true">#</a> <strong>例 1：注解写在 Controller 层的接口参数中</strong></h3>
<p><img src="@source/notes/record/参数校验注解不生效解决方案/assets/image-20230918153112631.png" alt="image-20230918153112631"></p>
<h3 id="例-2-注解写在实体中" tabindex="-1"><a class="header-anchor" href="#例-2-注解写在实体中" aria-hidden="true">#</a> <strong>例 2：注解写在实体中</strong></h3>
<p><img src="@source/notes/record/参数校验注解不生效解决方案/assets/image-20230918153141913.png" alt="image-20230918153141913"></p>
<blockquote>
<p>当这些注解不生效时，解决方案如下：</p>
</blockquote>
<p><strong>1、当@NotNull 等注解写在实体类中时，需要在 Controller 接口的方法参数加入@Valid 注解，如下图所示：</strong></p>
<p><img src="@source/notes/record/参数校验注解不生效解决方案/assets/image-20230918153232517.png" alt="image-20230918153232517"></p>
<p><strong>2、当@NotNull 等注解写在 Controller 层的接口参数中时，需要在该方法的 Controller 类上添加@Validated，如下图所示：</strong></p>
<p><img src="@source/notes/record/参数校验注解不生效解决方案/assets/image-20230918153315604.png" alt="image-20230918153315604"></p>
<h3 id="补充-除-notnull-之外的其他校验相关的注解" tabindex="-1"><a class="header-anchor" href="#补充-除-notnull-之外的其他校验相关的注解" aria-hidden="true">#</a> 补充：除@NotNull 之外的其他校验相关的注解</h3>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//被注释的元素必须为null</span>
