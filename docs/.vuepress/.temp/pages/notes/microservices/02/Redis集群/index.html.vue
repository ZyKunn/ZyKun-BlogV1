<template><div><h1 id="redis-集群-🧙" tabindex="-1"><a class="header-anchor" href="#redis-集群-🧙" aria-hidden="true">#</a> Redis 集群 🧙</h1>
<nav class="table-of-contents"><ul><li><router-link to="#_2-1-集群结构">2.1.集群结构</router-link></li><li><router-link to="#_2-2-准备实例和配置">2.2.准备实例和配置</router-link></li><li><router-link to="#_2-3-启动">2.3.启动</router-link></li><li><router-link to="#_2-4-开启主从关系">2.4.开启主从关系</router-link></li><li><router-link to="#_2-5-测试">2.5.测试</router-link></li><li><router-link to="#_3-1-集群结构">3.1.集群结构</router-link></li><li><router-link to="#_3-2-准备实例和配置">3.2.准备实例和配置</router-link></li><li><router-link to="#_3-3-启动">3.3.启动</router-link></li><li><router-link to="#_3-4-测试">3.4.测试</router-link></li><li><router-link to="#_4-1-集群结构">4.1.集群结构</router-link></li><li><router-link to="#_4-2-准备实例和配置">4.2.准备实例和配置</router-link></li><li><router-link to="#_4-3-启动">4.3.启动</router-link></li><li><router-link to="#_4-4-创建集群">4.4.创建集群</router-link></li><li><router-link to="#_4-5-测试">4.5.测试</router-link></li></ul></nav>
<p>本章是基于 CentOS7 下的 Redis 集群教程，包括：</p>
<ul>
<li>单机安装 Redis</li>
<li>Redis 主从</li>
<li>Redis 分片集群</li>
</ul>
<h1 id="_1-单机安装-redis" tabindex="-1"><a class="header-anchor" href="#_1-单机安装-redis" aria-hidden="true">#</a> 1.单机安装 Redis</h1>
<p>首先需要安装 Redis 所需要的依赖：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>然后将课前资料提供的 Redis 安装包上传到虚拟机的任意目录：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210629114325516.png" alt="image-20210629114325516"></p>
<p>例如，我放到了/tmp 目录：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210629114830642.png" alt="image-20210629114830642"></p>
<p>解压缩：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">tar</span> <span class="token parameter variable">-xvf</span> redis-6.2.4.tar.gz
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>解压后：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210629114941810.png" alt="image-20210629114941810"></p>
<p>进入 redis 目录：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">cd</span> redis-6.2.4
...
<div class="language-properties line-numbers-mode" data-ext="properties"><pre v-pre class="language-properties"><code><span class="token comment"># 绑定地址，默认是127.0.0.1，会导致只能在本地访问。修改为0.0.0.0则可以在任意IP访问</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>启动 Redis：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>redis-server redis.conf
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>停止 redis 服务：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>redis-cli <span class="token function">shutdown</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h1 id="_2-redis-主从集群" tabindex="-1"><a class="header-anchor" href="#_2-redis-主从集群" aria-hidden="true">#</a> 2.Redis 主从集群</h1>
<h2 id="_2-1-集群结构" tabindex="-1"><a class="header-anchor" href="#_2-1-集群结构" aria-hidden="true">#</a> 2.1.集群结构</h2>
<p>我们搭建的主从集群结构如图：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210630111505799.png" alt="image-20210630111505799"></p>
<p>共包含三个节点，一个主节点，两个从节点。</p>
<p>这里我们会在同一台虚拟机中开启 3 个 redis 实例，模拟主从集群，信息如下：</p>
<table>
<thead>
<tr>
...
<h2 id="_2-2-准备实例和配置" tabindex="-1"><a class="header-anchor" href="#_2-2-准备实例和配置" aria-hidden="true">#</a> 2.2.准备实例和配置</h2>
<p>要在同一台虚拟机开启 3 个实例，必须准备三份不同的配置文件和目录，配置文件所在目录也就是工作目录。</p>
<p>1）创建目录</p>
<p>我们创建三个文件夹，名字分别叫 7001、7002、7003：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如图：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210630113929868.png" alt="image-20210630113929868"></p>
<p>2）恢复原始配置</p>
<p>修改 redis-6.2.4/redis.conf 文件，将其中的持久化模式改为默认的 RDB 模式，AOF 保持关闭状态。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>3）拷贝配置文件到每个实例目录</p>
<p>然后将 redis-6.2.4/redis.conf 文件拷贝到三个目录中（在/tmp 目录执行下列命令）：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>4）修改每个实例的端口、工作目录</p>
<p>修改每个文件夹内的配置文件，将端口分别修改为 7001、7002、7003，将 rdb 文件保存位置都修改为自己所在目录（在/tmp 目录执行下列命令）：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>5）修改每个实例的声明 IP</p>
<p>虚拟机本身有多个 IP，为了避免将来混乱，我们需要在 redis.conf 文件中指定每一个实例的绑定 ip 信息，格式如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>每个目录都要改，我们一键完成修改（在/tmp 目录执行下列命令）：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_2-3-启动" tabindex="-1"><a class="header-anchor" href="#_2-3-启动" aria-hidden="true">#</a> 2.3.启动</h2>
<p>为了方便查看日志，我们打开 3 个 ssh 窗口，分别启动 3 个 redis 实例，启动命令：</p>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210630183914491.png" alt="image-20210630183914491"></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="_2-4-开启主从关系" tabindex="-1"><a class="header-anchor" href="#_2-4-开启主从关系" aria-hidden="true">#</a> 2.4.开启主从关系</h2>
<p>现在三个实例还没有任何关系，要配置主从可以使用 replicaof 或者 slaveof（5.0 以前）命令。</p>
<p>有临时和永久两种模式：</p>
<ul>
<li>
<p>修改配置文件（永久生效）</p>
<ul>
<li>在 redis.conf 中添加一行配置：<code v-pre>slaveof &lt;masterip&gt; &lt;masterport&gt;</code></li>
</ul>
</li>
<li>
<p>使用 redis-cli 客户端连接到 redis 服务，执行 slaveof 命令（重启后失效）：</p>
...
<p><strong><font color='red'>注意</font></strong>：在 5.0 以后新增命令 replicaof，与 salveof 效果一致。</p>
<p>这里我们为了演示方便，使用方式二。</p>
<p>通过 redis-cli 命令连接 7002，执行下面命令：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过 redis-cli 命令连接 7003，执行下面命令：</p>
...
<span class="token comment"># 执行slaveof</span>
slaveof <span class="token number">192.168</span>.126.129 <span class="token number">7001</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后连接 7001 节点，查看集群状态：</p>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210630201258802.png" alt="image-20210630201258802"></p>
<h2 id="_2-5-测试" tabindex="-1"><a class="header-anchor" href="#_2-5-测试" aria-hidden="true">#</a> 2.5.测试</h2>
<p>执行下列操作以测试：</p>
<ul>
<li>
<p>利用 redis-cli 连接 7001，执行<code v-pre>set num 123</code></p>
</li>
<li>
<p>利用 redis-cli 连接 7002，执行<code v-pre>get num</code>，再执行<code v-pre>set num 666</code></p>
</li>
<li>
<p>利用 redis-cli 连接 7003，执行<code v-pre>get num</code>，再执行<code v-pre>set num 888</code></p>
</li>
</ul>
<p>可以发现，只有在 7001 这个 master 节点上可以执行写操作，7002 和 7003 这两个 slave 节点只能执行读操作。</p>
<h1 id="_3-搭建哨兵集群" tabindex="-1"><a class="header-anchor" href="#_3-搭建哨兵集群" aria-hidden="true">#</a> 3.搭建哨兵集群</h1>
<h2 id="_3-1-集群结构" tabindex="-1"><a class="header-anchor" href="#_3-1-集群结构" aria-hidden="true">#</a> 3.1.集群结构</h2>
<p>这里我们搭建一个三节点形成的 Sentinel 集群，来监管之前的 Redis 主从集群。如图：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701215227018.png" alt="image-20210701215227018"></p>
<p>三个 sentinel 实例信息如下：</p>
<table>
<thead>
<tr>
<th>节点</th>
...
<h2 id="_3-2-准备实例和配置" tabindex="-1"><a class="header-anchor" href="#_3-2-准备实例和配置" aria-hidden="true">#</a> 3.2.准备实例和配置</h2>
<p>要在同一台虚拟机开启 3 个实例，必须准备三份不同的配置文件和目录，配置文件所在目录也就是工作目录。</p>
<p>我们创建三个文件夹，名字分别叫 s1、s2、s3：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 进入/tmp目录</span>
<span class="token builtin class-name">cd</span> /tmp
<span class="token comment"># 创建目录</span>
<span class="token function">mkdir</span> s1 s2 s3
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如图：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701215534714.png" alt="image-20210701215534714"></p>
<p>然后我们在 s1 目录创建一个 sentinel.conf 文件，添加下面的内容：</p>
...
dir "/tmp/s1"
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>解读：</p>
<ul>
<li><code v-pre>port 27001</code>：是当前 sentinel 实例的端口</li>
<li><code v-pre>sentinel monitor mymaster 192.168.150.101 7001 2</code>：指定主节点信息
<ul>
<li><code v-pre>mymaster</code>：主节点名称，自定义，任意写</li>
<li><code v-pre>192.168.150.101 7001</code>：主节点的 ip 和端口</li>
<li><code v-pre>2</code>：选举 master 时的 quorum 值</li>
</ul>
</li>
</ul>
<p>然后将 s1/sentinel.conf 文件拷贝到 s2、s3 两个目录中（在/tmp 目录执行下列命令）：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 方式一：逐个拷贝</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改 s2、s3 两个文件夹内的配置文件，将端口分别修改为 27002、27003：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_3-3-启动" tabindex="-1"><a class="header-anchor" href="#_3-3-启动" aria-hidden="true">#</a> 3.3.启动</h2>
<p>为了方便查看日志，我们打开 3 个 ssh 窗口，分别启动 3 个 redis 实例，启动命令：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 第1个</span>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701220714104.png" alt="image-20210701220714104"></p>
<h2 id="_3-4-测试" tabindex="-1"><a class="header-anchor" href="#_3-4-测试" aria-hidden="true">#</a> 3.4.测试</h2>
<p>尝试让 master 节点 7001 宕机，查看 sentinel 日志：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701222857997.png" alt="image-20210701222857997"></p>
<p>查看 7003 的日志：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701223025709.png" alt="image-20210701223025709"></p>
<p>查看 7002 的日志：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210701223131264.png" alt="image-20210701223131264"></p>
<h1 id="_4-搭建分片集群" tabindex="-1"><a class="header-anchor" href="#_4-搭建分片集群" aria-hidden="true">#</a> 4.搭建分片集群</h1>
<h2 id="_4-1-集群结构" tabindex="-1"><a class="header-anchor" href="#_4-1-集群结构" aria-hidden="true">#</a> 4.1.集群结构</h2>
<p>分片集群需要的节点数量较多，这里我们搭建一个最小的分片集群，包含 3 个 master 节点，每个 master 包含一个 slave 节点，结构如下：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702164116027.png" alt="image-20210702164116027"></p>
...
<h2 id="_4-2-准备实例和配置" tabindex="-1"><a class="header-anchor" href="#_4-2-准备实例和配置" aria-hidden="true">#</a> 4.2.准备实例和配置</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在/tmp 下准备一个新的 redis.conf 文件，内容如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修改每个目录下的 redis.conf，将其中的 6379 修改为与所在目录一致：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 进入/tmp目录</span>
<span class="token builtin class-name">cd</span> /tmp
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_4-3-启动" tabindex="-1"><a class="header-anchor" href="#_4-3-启动" aria-hidden="true">#</a> 4.3.启动</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过 ps 查看状态：</p>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702174255799.png" alt="image-20210702174255799"></p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">printf</span> <span class="token string">'%s\n'</span> <span class="token number">7001</span> <span class="token number">7002</span> <span class="token number">7003</span> <span class="token number">8001</span> <span class="token number">8002</span> <span class="token number">8003</span> <span class="token operator">|</span> <span class="token function">xargs</span> -I<span class="token punctuation">{</span><span class="token punctuation">}</span> <span class="token parameter variable">-t</span> redis-cli <span class="token parameter variable">-p</span> <span class="token punctuation">{</span><span class="token punctuation">}</span> <span class="token function">shutdown</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h2 id="_4-4-创建集群" tabindex="-1"><a class="header-anchor" href="#_4-4-创建集群" aria-hidden="true">#</a> 4.4.创建集群</h2>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>2）Redis5.0 以后</p>
...
<li><code v-pre>--replicas 1</code>或者<code v-pre>--cluster-replicas 1</code> ：指定集群中每个 master 的副本个数为 1，此时<code v-pre>节点总数 ÷ (replicas + 1)</code> 得到的就是 master 的数量。因此节点列表中的前 n 个就是 master，其它节点都是 slave 节点，随机分配到不同 master</li>
</ul>
<p>运行后的样子：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702181101969.png" alt="image-20210702181101969"></p>
<p>这里输入 yes，则集群开始创建：</p>
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702181215705.png" alt="image-20210702181215705"></p>
<p>通过命令可以查看集群状态：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702181922809.png" alt="image-20210702181922809"></p>
<h2 id="_4-5-测试" tabindex="-1"><a class="header-anchor" href="#_4-5-测试" aria-hidden="true">#</a> 4.5.测试</h2>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702182343979.png" alt="image-20210702182343979"></p>
...
<p><img src="@source/notes/microservices/02/Redis集群/assets/image-20210702182602145.png" alt="image-20210702182602145"></p>
</div></template>
