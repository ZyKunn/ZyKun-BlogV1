...
启动工程，尝试访问 http://localhost:63070/auth/r/r1 :
...
自动进入/login 登录页面，/login 是 spring security 提供的,此页面有几个 css 样式加载会稍微慢点，如下图：
...
比如：查询一个用户的信息，用户信息就是系统的资源，要访问资源需要通过 URL，所以我们在 controller 中定义的每个 http 的接口就是访问资源的接口。
...
FilterChainProxy 是一个代理，真正起作用的是 FilterChainProxy 中 SecurityFilterChain 所包含的各个 Filter，同时这些 Filter 作为 Bean 被 Spring 管理，它们是 Spring Security 核心，各有各的职责，但他们并不直接处理用户的**认证**，也不直接处理用户的**授权**，而是把它们交给了认证管理器（AuthenticationManager）和决策管理器（AccessDecisionManager）进行处理。
...
![image-20230604180541510](./assets/image-20230604180541510.png)
...
![image-20230604180611293](./assets/image-20230604180611293.png)
...
![image-20230604180746941](./assets/image-20230604180746941.png)
...
![image-20230604180830380](./assets/image-20230604180830380.png)
...
如果是基于令牌技术在分布式系统中实现认证则服务端不用存储 session，可以将用户身份信息存储在令牌中，用户认证通过后认证服务颁发令牌给用户，用户将令牌存储在客户端，去访问应用服务时携带令牌去访问，服务端从 jwt 解析出用户信息。这个过程就是无状态认证。
...
在认证服务中配置 jwt 令牌服务，即可实现生成 jwt 格式的令牌,
...
2、从资料中拷贝 资源服务配置类 中的 TokenConfig 和 ResouceServerConfig 到内容管理的 api 工程的 config 包下。
...
![image-20230604181331767](./assets/image-20230604181331767.png)
...
至此我们了解了使用 Spring Security 进行认证授权的过程，本节实现用户认证功能。
...
原来的 DaoAuthenticationProvider 会进行密码校验，现在重新定义 DaoAuthenticationProviders 类，重写类的 additionalAuthenticationChecks 方法。
...
此时可以重启认证服务，测试申请令牌接口，传入的账号信息改为 json 数据，如下：
...
![image-20230604182133296](./assets/image-20230604182133296.png)
...
注意修改 bootstrap.yml 中的命名空间为自己定义的命名空间。
...
![image-20230604182209500](./assets/image-20230604182209500.png)
...
勾选自动登录 cookie 生成时间为 30 天，不勾选自动登录关闭浏览器窗口后自动删除 cookie。
...
![image-20230604182444308](./assets/image-20230604182444308.png)
...
![image-20230604182646345](./assets/image-20230604182646345.png)
...
![image-20230604182827483](./assets/image-20230604182827483.png)

1、在内网穿透服务器上开通隧道，配置外网域名，配置穿透内网的端口即本地电脑上的端口。

![image-20230604182831376](./assets/image-20230604182831376.png)

这里我们配置认证服务端口，最终实现通过外网域名访问本地认证服务。

2、在本地电脑上安装内网穿透的工具，工具上配置内网穿透服务器隧道 token。
...
![image-20230604182837011](./assets/image-20230604182837011.png)
...
![image-20230604182911877](./assets/image-20230604182911877.png)

点击微信图标打开二维码

![image-20230604182917079](./assets/image-20230604182917079.png)
...
1、使用 restTemplate 请求微信，配置 RestTemplate bean
...
3、下边在 controller 中调用 wxAuth 接口：
...
![image-20230608211738183](./assets/image-20230608211738183.png)

配置 appid 和 秘钥

4、在 WxAuthService 的 wxAuth 方法中实现申请令牌、查询用户信息等内容。
...
![image-20230604183050092](./assets/image-20230604183050092.png)
...
![image-20230604183118856](./assets/image-20230604183118856.png)
...
这里使用 t1 用户登录，账号:t1、密码：111111

登录成功，点击“教学机构”

![image-20230604183204291](./assets/image-20230604183204291.png)

当用户没有权限时页面提示：没有操作此功能的权限。

![image-20230604183207873](./assets/image-20230604183207873.png)
...
![image-20230604183218592](./assets/image-20230604183218592.png)
...
![image-20230604183224065](./assets/image-20230604183224065.png)

本项目要求掌握基于权限数据模型（5 张数据表），要求在数据库中操作完成给用户分配权限、查询用户权限等需求。
...
使用 Spring Security 进行授权，首先在生成 jwt 前会查询用户的权限，如下图：

![image-20230604183238823](./assets/image-20230604183238823.png)

接下来需要修改 UserServiceImpl 和 PasswordAuthServiceImpl 从数据库查询用户的权限。
...
![image-20230604183306637](./assets/image-20230604183306637.png)

用户拥有 xc_teachmanager_course_list 权限方可访问课程查询接口。

以用户 stu1 为例，当它没有此权限时页面报“没有此操作的权限”错误

![image-20230604183309879](./assets/image-20230604183309879.png)
...
细粒度授权也叫数据范围授权，即不同的用户所拥有的操作权限相同，但是能够操作的数据范围是不一样的。一个例子：用户 A 和用户 B 都是教学机构，他们都拥有“我的课程”权限，但是两个用户所查询到的数据是不一样的。
...
在用户表中设计了 company_id 字段存储该用户所属的机构 id.

2）接口层取出当前登录用户的身份，取出机构 id

3\) 将机构 id 传入 service 方法。

4\) service 方法将机构 id 传入 Dao 方法，最终查询出本机构的课程信息。
...
我们准备了 t1 用户作为此次测试的用户，使用此用户登录系统：

提前在查询课程列表接口处打上断点。

经过测试可以正常取出用户所属的机构 id

![image-20230604183347236](./assets/image-20230604183347236.png)

跟踪持久层日志发现已将机构 id 传入 dao 方法，拼装 sql 语句，查询本机构的课程信息

![image-20230604183417411](./assets/image-20230604183417411.png)
...
![image-20230604183353300](./assets/image-20230604183353300.png)
...
![image-20230604183444148](./assets/image-20230604183444148.png)
